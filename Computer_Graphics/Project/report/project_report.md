...
And then labeling the dataset by human labor after we actually get dataset in real world is costly, time-consuming and error-prone. So it would be great if we can get repeatable, accurate, low-cost, easy to get dataset and it would be better if we can configure the environments as we wish, and the synthetic way of generating dataset then train on them is getting more and more attention recent years and is a good candidate for our purpose, like [... ref].
...
The experiment is pretty easy to follow under the official guidance, in Figure \ref{fig:gym_unrealcv_results}, we provide the result of one experiments, they are results of the 0-100, 600-700, 1200-1300, 1800-1900, and 2400-2500 epochs respectively.
...
So, it spent around 2182/60=36.37 minutes to generate 8000 images, much more quicker than in real world environments, the generated files are around 4.4GB. And Figure \ref{...} are a screenshots of the generated images.
...
And we then digest the failed ones, we list [... 20 images failed]. There is a common character that looking the soft in a no frontal down view will probably not produce nice results. Great chances are that the training dataset used for training may lack this kind of view of sofa.
